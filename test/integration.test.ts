...
import { buildXkb, generateKeycodes, generateKeyTypes, generateSymbols } from "../src/index.js";
...
const MINIMAL_KEYMAP = `xkb_keymap {
...
function runXkbcomp(input: string): { ok: boolean; output: string } {
  const tmpFile = join(tmpdir(), `jsxcb-test-${Date.now()}-${Math.random().toString(36).slice(2)}.xkb`);
  const outFile = join(tmpdir(), `jsxcb-test-${Date.now()}-${Math.random().toString(36).slice(2)}.xkm`);
  try {
...
describe("xkbcomp integration", () => {
  it.skipIf(!HAS_XKBCOMP)("validates minimal XKB keymap with xkbcomp", () => {
    const xkb = buildXkb();
    const tmpFile = join(tmpdir(), `jsxcb-test-${Date.now()}.xkb`);
    const outFile = join(tmpdir(), `jsxcb-test-${Date.now()}.xkm`);

    try {
      writeFileSync(tmpFile, xkb, "utf-8");
